...
import { createUser, findUserByEmail, findUsers, findUsersCount, updateUser } from '@infrastructure/data-access/user.data-access';
...
const handler: NextApiHandler = async (req, res) => {
	...
	if (req.method === 'GET') {
		...
		const users = await findUsers(searchRequest, sortParams, Number(skip), Number(limit));
	...
	if (req.method === 'PUT') {
		...
		const updatedUser = await updateUser({ ...userToUpdate }, true);
